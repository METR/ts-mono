...
export function VirtualList<T>({
  ...
  // Also the list's own root, which the match locator uses to reject a
  // selection belonging to a different list mounted alongside this one.
  ...
  const searchInData = useCallback<ExtendedFindFn>(
    (term, direction, onContentReady) => {
      ...
      if (!term || len === 0 || !findContext) return Promise.resolve(false);
    ...
    [precomputedSearchTexts, settleScrollToIndex, findContext]
  );
  ...
  const locateInData = useCallback<MatchLocatorFn>(
    (term) => {
      ...
      const at = itemOccurrenceAtSelection(wrapperRef.current, lowerTerm);
